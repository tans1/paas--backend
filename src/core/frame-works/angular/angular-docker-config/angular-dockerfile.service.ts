...
export class AngularDockerfileService {
  ...
  async createDockerfile(projectConfig: {
    ...
    try {
      const { ssrEnabled, entryFile } =
        await this.determineSSREntry(projectPath);
  ...
  private async checkForSSR(projectPath: string): Promise<boolean> {
    ...
    try {
      ...
      const angularJson = JSON.parse(
        await fs.promises.readFile(angularJsonPath, 'utf-8'),
      );
      ...
      for (const projectName of Object.keys(angularJson.projects)) {
        ...
        const serverTarget =
          project.architect?.server || project.targets?.server;
        ...
        if (
          productionConfig?.ssr ||
          productionConfig?.outputMode === 'server'
        ) {
          return true;
        }
      }
  ...
  private async determineSSREntry(
    projectPath: string,
  ): Promise<{ ssrEnabled: boolean; entryFile?: string }> {
    try {
      ...
      try {
        ...
        moduleFormat =
          tsconfig.compilerOptions?.module?.toLowerCase() || moduleFormat;
      } catch {
  ...
  private getEntryExtension(
    angularVersion: semver.SemVer | null,
    moduleFormat: string,
  ): 'js' | 'mjs' {
    const normalizedModule = moduleFormat.toLowerCase();

    const isESModule =
      /^es\d{4}$|^esnext$/i.test(moduleFormat) ||
      normalizedModule.includes('esm');
